...
ADR review completed for this change. The architectural convention of ephemeral secret seed generation, deterministic role credential derivation, dynamic PostgreSQL role initialization via shell script, and Git exclusion of local environment files has been codified in repository-level ADR 0015, amending Decision 2 of ADR 0007.
...
- [0015-docker-compose-secret-derivation-and-isolation.md](../../../../adr/0015-docker-compose-secret-derivation-and-isolation.md): Codifies ephemeral master secret seed generation in Gradle, deterministic role password derivation, dynamic PostgreSQL role initialization via `01-init.sh`, and `.env` Git exclusion.
