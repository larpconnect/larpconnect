...
ADR review completed for this change. The architectural convention of encapsulating Picocli nullness taint at the command boundary and exposing typed `Optional<T>` accessors and option records has been codified in repository-level ADR 0012.
...
- [0012-cli-command-option-null-encapsulation-and-optional-accessors.md](../../../../adr/0012-cli-command-option-null-encapsulation-and-optional-accessors.md): Mandates `Optional<T>` return types for public CLI command accessors and options record components, isolating Picocli `@Nullable` fields strictly to internal reflection targets.
