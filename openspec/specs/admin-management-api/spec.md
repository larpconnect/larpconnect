...
The system SHALL expose HTTP REST endpoints under `/api/admin/v1/roles` allowing administrators to create, list, and retrieve roles. The `POST /api/admin/v1/roles` endpoint SHALL accept a JSON payload with `roleName` string, validate that `roleName` is strictly alphanumeric lowercase and underscores starting with a lowercase letter (`^[a-z][a-z0-9_]*$`), generate a UUIDv7 identifier, and return HTTP 201 Created. If the role name format is invalid, the system SHALL return HTTP 400 Bad Request. The `GET /api/admin/v1/roles` endpoint SHALL return all registered roles. The `GET /api/admin/v1/roles/{id}` endpoint SHALL accept either role UUID or `roleName` string, returning HTTP 200 OK or HTTP 404 Not Found.
...
#### Scenario: Reject invalid role name format
- **GIVEN** an administrative client
- **WHEN** the client sends a POST request to `/api/admin/v1/roles` with body `{"roleName": "123_invalid"}`
- **THEN** the system returns HTTP 400 Bad Request with an AIP-193 structured JSON error object
