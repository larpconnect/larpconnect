...
description: Common patterns to successfully follow the coding standards for this project.
---
...
1. Modify the `openapi.yaml` file to add the endpoint and specify the payload for both the call and response.
2. Add the verticle to handle the JSON version of the request in `:api`. It MAY delegate
...
3. Wire it in the `:server` module.
4. Write unit tests.
5. Write integration tests.

### Database modifications

1. Use flyway to create _versioned_ (prefixing with a number) database scripts.
