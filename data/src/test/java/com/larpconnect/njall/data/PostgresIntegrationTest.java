...
@Disabled("Docker environment incompatible on build agent")
@Testcontainers
@ExtendWith(VertxExtension.class)
class PostgresIntegrationTest {
  static {
    java.lang.System.setProperty("DOCKER_API_VERSION", "1.44");
  }
  ...
  private static final PostgreSQLContainer<?> POSTGRES =
      new PostgreSQLContainer<>(
              DockerImageName.parse(
                  "postgres:16")) // PSQL 16 as Testcontainers might not fully support 18 yet
  ...
  static void setUp() {
    ...
    sessionFactory =
        Persistence.createEntityManagerFactory("njall-pu", properties)
  ...
  @Test
  void saveEntity_whenValid_shouldPersist(Vertx vertx, VertxTestContext testContext) {
    ...
    sessionFactory
        .withTransaction((session, tx) -> session.persist(metadata))
        .subscribe()
        .with(
            v ->
                testContext.verify(
