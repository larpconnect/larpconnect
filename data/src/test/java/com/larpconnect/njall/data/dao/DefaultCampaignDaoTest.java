...
class DefaultCampaignDaoTest {
  ...
  void setUp() {
    ...
    dao = new DefaultCampaignDao(sessionFactoryProvider);
  }

  @Test
  void findById_shouldReturnEntity_whenFound() {
    UUID id = null;
    Campaign expectedEntity = org.mockito.Mockito.mock(Campaign.class);

    when(sessionFactory.withSession(any()))
        .thenAnswer(
            invocation -> {
              Function<Mutiny.Session, Uni<Campaign>> function = invocation.getArgument(0);
              return function.apply(session);
            });
    when(session.find(Campaign.class, id)).thenReturn(Uni.createFrom().item(expectedEntity));

    Campaign actualEntity = dao.findById(id).await().indefinitely();

    assertThat(actualEntity).isEqualTo(expectedEntity);
    verify(session).find(Campaign.class, id);
  }

  @Test
  void persist_shouldReturnPersistedEntity() {
    Campaign entityToPersist = org.mockito.Mockito.mock(Campaign.class);
    ...
    Campaign persistedEntity = dao.persist(entityToPersist).await().indefinitely();
