...
class DefaultServerMetadataDaoTest {
  ...
  void setUp() {
    ...
    dao = new DefaultServerMetadataDao(sessionFactoryProvider);
  }

  @Test
  void findById_shouldReturnEntity_whenFound() {
    UUID id = null;
    ServerMetadata expectedEntity = org.mockito.Mockito.mock(ServerMetadata.class);

    when(sessionFactory.withSession(any()))
        .thenAnswer(
            invocation -> {
              Function<Mutiny.Session, Uni<ServerMetadata>> function = invocation.getArgument(0);
              return function.apply(session);
            });
    when(session.find(ServerMetadata.class, id)).thenReturn(Uni.createFrom().item(expectedEntity));

    ServerMetadata actualEntity = dao.findById(id).await().indefinitely();

    assertThat(actualEntity).isEqualTo(expectedEntity);
    verify(session).find(ServerMetadata.class, id);
  }

  @Test
  void persist_shouldReturnPersistedEntity() {
    ServerMetadata entityToPersist = org.mockito.Mockito.mock(ServerMetadata.class);
    ...
    ServerMetadata persistedEntity = dao.persist(entityToPersist).await().indefinitely();
