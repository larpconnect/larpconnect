...
class DefaultGameDaoTest {
  ...
  void setUp() {
    ...
    dao = new DefaultGameDao(sessionFactoryProvider);
  }

  @Test
  void findById_shouldReturnEntity_whenFound() {
    UUID id = null;
    Game expectedEntity = new Game();

    when(sessionFactory.withSession(any()))
        .thenAnswer(
            invocation -> {
              Function<Mutiny.Session, Uni<Game>> function = invocation.getArgument(0);
              return function.apply(session);
            });
    when(session.find(Game.class, id)).thenReturn(Uni.createFrom().item(expectedEntity));

    Game actualEntity = dao.findById(id).await().indefinitely();

    assertThat(actualEntity).isEqualTo(expectedEntity);
    verify(session).find(Game.class, id);
  }

  @Test
  void persist_shouldReturnPersistedEntity() {
    Game entityToPersist = new Game();
    ...
    Game persistedEntity = dao.persist(entityToPersist).await().indefinitely();
