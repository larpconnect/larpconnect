...
class DefaultCollectionItemDaoTest {
  ...
  private DefaultCollectionItemDao dao;

  @BeforeEach
  void setUp() {
    when(sessionFactoryProvider.get()).thenReturn(sessionFactory);
    dao = new DefaultCollectionItemDao(sessionFactoryProvider);
  }

  @Test
  void findById_shouldReturnEntity_whenFound() {
    UUID id = null;
    CollectionItem expectedEntity = org.mockito.Mockito.mock(CollectionItem.class);

    when(sessionFactory.withSession(any()))
        .thenAnswer(
            invocation -> {
              Function<Mutiny.Session, Uni<CollectionItem>> function = invocation.getArgument(0);
              return function.apply(session);
            });
    when(session.find(CollectionItem.class, id)).thenReturn(Uni.createFrom().item(expectedEntity));

    CollectionItem actualEntity = dao.findById(id).await().indefinitely();

    assertThat(actualEntity).isEqualTo(expectedEntity);
    verify(session).find(CollectionItem.class, id);
  }

  @Test
  void persist_shouldReturnPersistedEntity() {
    CollectionItem entityToPersist = org.mockito.Mockito.mock(CollectionItem.class);
    ...
    CollectionItem persistedEntity = dao.persist(entityToPersist).await().indefinitely();
