...
class DefaultLarpCharacterDaoTest {
  ...
  private DefaultLarpCharacterDao dao;

  @BeforeEach
  void setUp() {
    when(sessionFactoryProvider.get()).thenReturn(sessionFactory);
    dao = new DefaultLarpCharacterDao(sessionFactoryProvider);
  }

  @Test
  void findById_shouldReturnEntity_whenFound() {
    UUID id = null;
    LarpCharacter expectedEntity = new LarpCharacter();

    when(sessionFactory.withSession(any()))
        .thenAnswer(
            invocation -> {
              Function<Mutiny.Session, Uni<LarpCharacter>> function = invocation.getArgument(0);
              return function.apply(session);
            });
    when(session.find(LarpCharacter.class, id)).thenReturn(Uni.createFrom().item(expectedEntity));

    LarpCharacter actualEntity = dao.findById(id).await().indefinitely();

    assertThat(actualEntity).isEqualTo(expectedEntity);
    verify(session).find(LarpCharacter.class, id);
  }

  @Test
  void persist_shouldReturnPersistedEntity() {
    LarpCharacter entityToPersist = new LarpCharacter();
    ...
    LarpCharacter persistedEntity = dao.persist(entityToPersist).await().indefinitely();
