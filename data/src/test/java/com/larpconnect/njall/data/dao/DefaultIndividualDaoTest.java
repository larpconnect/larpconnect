...
class DefaultIndividualDaoTest {
  ...
  void setUp() {
    ...
    dao = new DefaultIndividualDao(sessionFactoryProvider);
  }

  @Test
  void findById_shouldReturnEntity_whenFound() {
    UUID id = null;
    Individual expectedEntity = org.mockito.Mockito.mock(Individual.class);

    when(sessionFactory.withSession(any()))
        .thenAnswer(
            invocation -> {
              Function<Mutiny.Session, Uni<Individual>> function = invocation.getArgument(0);
              return function.apply(session);
            });
    when(session.find(Individual.class, id)).thenReturn(Uni.createFrom().item(expectedEntity));

    Individual actualEntity = dao.findById(id).await().indefinitely();

    assertThat(actualEntity).isEqualTo(expectedEntity);
    verify(session).find(Individual.class, id);
  }

  @Test
  void persist_shouldReturnPersistedEntity() {
    Individual entityToPersist = org.mockito.Mockito.mock(Individual.class);
    ...
    Individual persistedEntity = dao.persist(entityToPersist).await().indefinitely();
