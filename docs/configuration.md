...
### JSON Key Names and Protobuf

When configuring the application via `config.json`, the configuration keys must map to the
standard Protobuf JSON representation of the properties defined in `.proto` files (e.g.,
`webPort` or `web_port` instead of `web.port`). This is because Guice uses
`JsonFormat.parser().ignoringUnknownFields().merge()` to deserialize the JSON into the
internal configuration object. Unknown fields like `web.port` are silently ignored.
